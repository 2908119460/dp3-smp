...
class SMPDP3Policy(BasePolicy):
    """DP3 observations with SMP diffusion in orthogonal skill space."""
    ...
    def _prepare_observation(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        prepared = dict(obs)
    ...
    def _task_tensor(self, batch_size: int, device: torch.device,
    ...
    def compute_loss(self, batch):
        ...
        # 论文 Eq. (36)：对 stop-gradient basis 得到的干净系数重新加噪。
        ...
        # 论文 Eq. (36)：系数维平方范数与时间维均求和，仅对 batch 取均值。
        coeff_loss = F.mse_loss(
            prediction, noise, reduction="none").sum(dim=(1, 2)).mean()
        # 论文 Eq. (35)：L_recon = sum_t ||a_t-a_hat_t||^2 / (2*sigma_a^2)。
        ...
        # 论文 Eq. (38)：全局、初始和时间 sticky Dirichlet KL。
        ...
        # 论文 Eq. (40)：训练 posterior 与仅依赖状态的部署 router 对齐。
        align_loss = router_alignment_kl(
            targets["posterior_concentration"],
            prior_concentration,
        )
        # 论文 Algorithm 1 第 10 行为四项直接相加；这里的权重属于 DP3 工程缩放。
        total_loss = (
    ...
    def predict_action(
        ...
        for timestep in self.noise_scheduler.timesteps:
            prediction = self.model.smp.denoise_coefficients(
                coefficient, timestep, state_feature)
